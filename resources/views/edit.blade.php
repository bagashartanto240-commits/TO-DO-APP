        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="glass-card shadow-lg">
                    ...
                    <form action="{{ route('todo.update_data', $todo->id) }}" method="POST">
                        @csrf 
                        @method('PUT')
                        
                        <div class="mb-3">
                            <label class="small fw-bold text-muted">Title</label>
                            <input type="text" name="judul" class="form-control" value="{{ $todo->judul }}" required 
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" name="deadline" class="form-control" 
                                   value="{{ $todo->deadline ? date('Y-m-d\TH:i', strtotime($todo->deadline)) : '' }}" 
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="keterangan" class="form-control" rows="4" 
                                      style="background: rgba(0,0,0,0.05); border: none; padding: 12px;">{{ $todo->keterangan }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <a href="{{ route('todo.index') }}" class="btn btn-light w-100 py-3 fw-bold shadow-sm">BACK</a>
